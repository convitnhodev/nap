...
func TestNapNew(t *testing.T) {
	...
	cases := []*Nap{
		&Nap{httpClient: &http.Client{}, method: "GET", rawURL: "https://example.com"},
		&Nap{httpClient: nil, method: "", rawURL: "https://example.com"},
		&Nap{queryStructs: make([]interface{}, 0)},
		&Nap{queryStructs: []interface{}{paramsA}},
		&Nap{queryStructs: []interface{}{paramsA, paramsB}},
		&Nap{bodyProvider: fakeBodyProvider},
		&Nap{bodyProvider: fakeBodyProvider},
		&Nap{bodyProvider: nil},
	...
	for _, nap := range cases {
		...
		// Header should be a copy of parent Nap header. For example, calling
