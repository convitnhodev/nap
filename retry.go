...
func NewRetryDoer(doer Doer, log l.Logger, opts ...RetryOption) *RetryDoer {
	if doer == nil {
		doer = defaultClient
	}
...
func (c *RetryDoer) DoCustom(req *Request) (*http.Response, error) {
	c.log.Info("performing request", l.String("method", req.Method), l.String("url", req.URL.String()))
	...
	for i := 0; ; i++ {
		...
		if doErr != nil {
			c.log.Error("request failed", l.String("method", req.Method), l.String("url", req.URL.String()), l.Error(doErr))
		}
		...
		if doErr == nil {
			...
			if err != nil {
				c.log.Error("error reading response body", l.Error(err))
			}
		}
		...
		c.log.Info("retrying request", l.String("request", desc), l.String("timeout", wait.String()), l.Int("remaining", remain))
		select {
